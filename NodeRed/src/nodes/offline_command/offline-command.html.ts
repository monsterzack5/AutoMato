...
import type { OfflineCommandRegisterType } from "../../@types/node_types";

import { updateModuleFunctionsAutofill, updateModulesAutofill } from "../common_browser/autofill";

declare const RED: EditorRED;

RED.nodes.registerType<OfflineCommandRegisterType>("offline-command", {
    category: "Automato",
    color: "#a6bbcf",
    // TODO:
    ...
    label() {
        ...
        // console.log("Running label() on offline-command");
        ...
        return this.name ?? "Offline Command";
    },
    async onpaletteadd() {
    ...
    async oneditprepare() {
        ...
        if (typeof loadedModules === "string") {
            ...
            console.log("offline-command.html::oneditprepare Error! loadedModules !== string");
        ...
        $("#node-input-module_name").on("change", async () => {
            ...
            } else console.log("offline-command.html::oneditprepare ERROR!\nnewModule !== string");
        });
    },
});
